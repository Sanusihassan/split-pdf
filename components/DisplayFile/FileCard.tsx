// this is the FileCard component:
...
const RangeFileCard = ({
  ...
  return (
    <div
      className={`position-relative range-file-card${fromToEqual ? " one-item-grid" : ""}`}
    >
      {fromToEqual ?
...
// this component rerenders it's parent when it changes the selectedPages state on the dispatch(setSelectedP..
const ExtractFileCard = React.memo(({ index }: {
  index: number;
}) => {
  const [showMark, setShowMark] = useState(false);
  // const state = useSelector((state: { tool: ToolState }) => state.tool);
  const selectedpages = useSelector((state: { tool: ToolState }) => state.tool.selectedPages);
  ...
  return (
    <div onClick={() => {
      ...
      if (selectedpages != "all") {
        dispatch(setSelectedPages(`${selectedpages?.length === 0 ? index + 1 : "," + index + 1}`));
      }
      ...
      if (extension && extension === ".pdf") {
        if (isSubscribed) {
          if (layout == "extract") {
            ...
            if (from && to && from === to) {
              let startUrl = await getNthPageAsImage(file, dispatch, errors, from);
          ...
          layout == "extract" ?
            imageUrls.map((imageUrl, index) => (
              ...
              <>
                ...
                <RangeFileCard image1Url={imageUrls[0]} image2Url={imageUrls[1]} loader_text={loader_text} fromToEqual={from && to && (from === to)} />
              </>
            )}
      </div >
    </>
